...
季節の札(春・夏・秋・冬・新年)で切り替え、分類で絞り込む。検索欄に文字を入れると、季節をまたいで見出し語・読み・傍題から探す。例えば「こがらし」で「木枯」が、「蝉時雨」で「蝉」が見つかる。各季語には読み・傍題・短い説明が付く。選んだ季節・分類・検索語はURLのハッシュに載るので、`#/?season=autumn&cat=shokubutsu` のように開くと同じ画面に戻れ、ブックマークや共有もできる。何も指定せずに開いたときは、その日の時季に合った季節(例えば九月なら秋)で始まる。`/` キーを押すと検索欄に移る。

### 詠草帳に書き留める

本文・形式(俳句・短歌)・季語・日付・覚え書きを書き留める。季語の欄は歳時記の見出し語から補完が効き、無季の句は空のままでよい。一覧では季語の札に季節の色が付く。各詠草には書き写す(本文をクリップボードへ)・直す(フォームに読み込んで上書き)・削除のボタンが付く。

詠草が増えてきたら、形式・季語の季節(無季を含む)・本文や覚え書きの語で絞り込み、新しい順と古い順を切り替えて見渡せる。書き出しは二通りで、「JSON」は取り込みで読み戻せるバックアップ、「テキスト」は本文・よみ・形式・季語・日付・覚え書きを綴った、印刷や共有に向く読み物としての書き出し(古い順)である。「取り込む」はJSONを読み戻し、同じ詠草を重複させず未知のものだけを足すので、端末をまたいで詠草を持ち運べる。
...
- `src/lib/poems.ts` — 詠草の型・検証・並べ替え・絞り込み・取り込み統合・テキスト書き出し・永続化
- `src/lib/season.ts` — 日付から「いまの季節」を割り出す
