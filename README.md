...
詠草が増えてきたら、形式・季語の季節(無季を含む)・本文や覚え書きの語で絞り込み、新しい順と古い順を切り替えて見渡せる。「書き出す」で全詠草をJSONとして保存でき、「取り込む」で読み戻す。取り込みは同じ詠草を重複させず、未知のものだけを足すので、端末をまたいで詠草を持ち運べる。
...
- `src/lib/poems.ts` — 詠草の型・検証・並べ替え・絞り込み・取り込み統合・永続化
